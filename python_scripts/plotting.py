...
## Metrics
from ipynb.fs.defs.metrics import compute_score, tpr_from_dset, fpr_from_dset, roc_auc_score, pr_auc_score
## Utils
from ipynb.fs.defs.utils import (filter_dates_like, 
                                 final_day_of_month, 
                                 MidpointNormalize, 
                                 format_fn_y, 
...
def show_multiple_time_series(dset):
    ...
    for var_name in var_names:
        ...
        plt.title(var_name)
        if dictionary['server'] == 'CFC': plt.show()
        plt.close()
...
def show_hyperparameters(tgn):
    ...
    if dictionary['cv_type'] == 'none': 
        ...
        if dictionary['verbosity'] > 1 and dictionary['interface'] == 'jupyterlab':
            # Show table
...
def plot_pred_time_series(y, predictions_mlr, predictions_rfr, persistence, climatology, ecmwf, lead_time, tgn, outer_split_num_ = None):
    ...
    for i, ax in enumerate(axes):
        ...
        # ML predictions
        if dictionary['reg_regr']: ax.plot(predictions_mlr_df, '--', label = 'RR', color = 'crimson')
        else: ax.plot(predictions_mlr_df, '--', label = 'MLR', color = 'crimson')
    ...
    if dictionary['verbosity'] > 0 and dictionary['server'] == 'CFC': plt.show()
    plt.close()
...
def plot_zoomed_in_pred_time_series(y, predictions_mlr, predictions_rfr, persistence, climatology, ecmwf, start_date_test, lead_time, tgn, outer_split_num_ = None):
    ...
    # ML predictions
    if dictionary['reg_regr']: plt.plot(predictions_mlr_df.loc[zoom_period_start:zoom_period_end], '--', label = 'RR', color = 'crimson')
    else: plt.plot(predictions_mlr_df.loc[zoom_period_start:zoom_period_end], '--', label = 'MLR', color = 'crimson')
...
def plot_pred_time_series_all_lead_times():
    ...
    for lt in dictionary['lead_times']:
        ...
        pred_mlr_full, pred_mlr_ensemble, pred_rfr_full, pred_rfr_ensemble = read_old_regr_ml_forecasts('t2m', lt)
        pred_mlr = pred_mlr_full['test']
        ...
        predictions_mlr = pd.DataFrame(data = pred_mlr, index = index).sort_index()  
        ...
        for i, ax in enumerate(axes[lt - 1,:]):
            ...
            # ML predictions
            if dictionary['reg_regr']: ax.plot(predictions_mlr, '--', label = 'RR', color = 'crimson')
            else: ax.plot(predictions_mlr, '--', label = 'MLR', color = 'crimson')
    ...
    ## Save plot
    dir_name = dictionary['path_plots_prediction'] + 'test/regr/time_series/'
...
def plot_cross_corr(y, predictions_mlr, predictions_rfr, predictions_ecmwf, persistence, lead_time, tgn, outer_split_num_ = None):
    ...
    ## MLR
    plt.xcorr(y, predictions_mlr, maxlags = maxlagnum, normed = True, color = 'crimson', label = 'MLR-Ground Truth', linewidth = 1, usevlines = False, linestyle = '-', marker = 'o', markersize = 5)
    plt.xcorr(persistence, predictions_mlr, maxlags = maxlagnum, normed = True, color = 'crimson', label = 'MLR-Persistence', linewidth = 1, usevlines = False, linestyle = '--', marker = 'o', markerfacecolor = 'none', markersize = 5)
    ## RFR
...
def plot_metrics(metrics_full, prediction_type):
    ...
    if prediction_type == 'regr': 
        ...
        for metric, index in zip(metrics_names, range(1, columns * rows + 1)):
            ...
            if prediction_type == 'regr': 
                ...
                # Change MLR to RR for regularized case
                if dictionary['reg_regr']: forecasts_labels = reference_models + ['RR', 'RFR']
   ...
   # Save plot
    dir_name = dictionary['path_plots_prediction'] + 'test/' + prediction_type + '/metrics_plot/'
...
def show_coeffs(_model_, model_name, _pred_names, _feature_names, _tgn_, _lead_time_, outer_split_num_ = None):
    ...
    if dictionary['cv_type'] == 'none' and dictionary['server'] == 'CFC': 
        ...
        if _lead_time_ == dictionary['lead_times'][-1] and dictionary['verbosity'] > 1 and dictionary['interface'] == 'jupyterlab':            
            # Show table
            pd.set_option('display.max_rows', None, 'display.max_columns', None)
            display(df)
            pd.reset_option('display.max_rows', 'display.max_columns')
...
def plot_random_forest_overfitting(X_train_, y_train_, X_vali_, y_vali_, optimize_metric_, tgn_, _lead_time_, balance_, best_n_estimators_, best_max_depth_, best_min_samples_split_, best_min_samples_leaf_, best_bootstrap_, outer_split_num_ = None):
    ...
    for min_samples_leaf_ in mean_sample_leaves_: #25 
        if min_samples_leaf_ > 1:
            ...
            ### Plot points
            plt.scatter(min_samples_leaf_, score_new_train, marker = 'o', c = 'orange', label = 'Train')
            plt.scatter(min_samples_leaf_, score_new_vali, marker = 'o', c = 'turquoise', label = 'Validation')
    ...
    ## Custom legend
    legend_elements = [Line2D([0], [0], marker = 'o', color = 'orange', label = 'Train', markersize = 5),
                   Line2D([0], [0], marker = 'o', color = 'turquoise', label = 'Validation', markersize = 5)]
...
def plot_scatter_diagrams(X, y, tgn_):
    ...
    for predictor in predictor_names:
        plt.scatter(X[predictor], y, color = 'orange', s = 2, marker = 'o')
        if dictionary['plot_fig_title'] == True:
...
def plot_var_expl(var_expl_, num_pc_choosen, _tgn_, _lead_time_, outer_split_num_ = None):    
    ...
    plt.bar(['PC{}'.format(i) for i in range(1, num_pc_choosen + 1)], var_expl_[:num_pc_choosen], orientation = 'vertical', color = 'orange', label = 'Selected PCs')
    plt.xticks(rotation = 90)
...
def plot_data_histogram(t2m_anom, num_indices):
    
    """
      inputs
      -------
        t2m_anom                             xr.DataArray : 2m temperature anomalies in Central European box (time series)
        num_indices                                   int : number of indices included (1-3) in the following order: 1.5SD, 1SD, 2in7
      ...
      -------
        ...
        standarized (μ = 0, σ = 1) temperature anomalies. The vertical blue line marks the mean (μ = 0) of the distribution. The stippled turquoise (red) 
        line marks +1 (+1.5) standard deviations (σ) from the mean. The +1σ (+1.5σ) index is defined as 1 for the events found on the right of the turquoise 
    ...
    n, bins, patches = plt.hist(t2m_anom.values, 200, facecolor = 'orange', alpha = 0.75, density = False)
    plt.xlabel('Standardized temperature anomalies ($\sigma$)', fontsize = 16)
    plt.ylabel('Number of events', fontsize = 16)
    ### Best fit of data
    (mu, sigma) = norm.fit(t2m_anom)
    plt.axvline(np.round(mu, decimals = 0), color = 'orange', label = 'Std T$_{anom}$ $\mu$=%.0f' %(abs(mu)))
    ...
    plt.axvline(1, color = 'darkturquoise', label = 'Std T$_{anom}$ at +1$\sigma$', linestyle = '--')
